...
export class OpenAiLlmService extends LlmService {
  ...
  async answerMessage(
    ...
    try {
      ...
      if (
        response.output_parsed.action.type === 'suggest_carts' &&
        !response.output_parsed.action.payload?.input?.trim()
      ) {
        console.error(
          'Invalid LLM response: suggest_carts requires payload.input',
        );
        return null;
      }
